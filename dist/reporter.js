...
function buildComment(report) {
    ...
    if (report.critical.length)
        lines.push(`| ${SEVERITY_EMOJI.critical} Critical | ${report.critical.length} |`);
    if (report.high.length)
        lines.push(`| ${SEVERITY_EMOJI.high} High     | ${report.high.length} |`);
    if (report.medium.length)
        lines.push(`| ${SEVERITY_EMOJI.medium} Medium   | ${report.medium.length} |`);
    if (report.low.length)
        lines.push(`| ${SEVERITY_EMOJI.low} Low      | ${report.low.length} |`);
    for (const [label, findings] of [
        ...
        if (!findings.length)
            continue;
        lines.push(`\n<details><summary>${SEVERITY_EMOJI[label.toLowerCase()]} ${label} (${findings.length})</summary>\n`);
